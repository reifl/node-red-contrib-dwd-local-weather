module.exports = function (RED) {
  ...
  function updateWeatherForecastIfOutdated(node) {
    if (
      !nextWeatherUpdate[node.mosmixStation] ||
      new Date().getTime() > nextWeatherUpdate[node.mosmixStation]
    ) {
  ...
  function updateWeatherForecast(node) {
    ...
    xmlStreamParser.onopentag = (tag) => {
      if (!isInitialized) {
        ...
        nextWeatherUpdate[node.mosmixStation] =
          new Date().getTime() + MOSMIX_MAXAGE;
        isInitialized = true;
      }
    ...
    xmlStreamParser.ontext = (text) => {
      if (xmlTagStack.length) {
        ...
        if (xmlTagStack.length >= 2 && currentTag.name == "dwd:value") {
          ...
          if (
            enclosingTag.name == "dwd:Forecast" &&
            enclosingTag.attributes["dwd:elementName"] &&
            mosmixElements[node.mosmixStation].includes(
              enclosingTag.attributes["dwd:elementName"]
            )
          ) {
            weatherForecast[node.mosmixStation][
              enclosingTag.attributes["dwd:elementName"]
            ] = text.split(/\s+/).map((v) => Number.parseFloat(v));
          }
        }
      }
    };
    ...
    node.status({
      fill: "blue",
      shape: "dot",
      text: "dwdweather.status.requesting",
    });
  ...
  function getInterpolatedValue(mosmixStation, attribute, forecastDate) {
    ...
    if (idx == 0) {
      ...
      Number.isNaN(weatherForecast[mosmixStation][attribute][idx - 1])
    ) {
      ...
      var share =
        (forecastDate.getTime() -
          weatherForecast[mosmixStation].times[idx - 1].getTime()) /
        (weatherForecast[mosmixStation].times[idx].getTime() -
          weatherForecast[mosmixStation].times[idx - 1].getTime());
      return (
        weatherForecast[mosmixStation][attribute][idx - 1] +
        share *
          (weatherForecast[mosmixStation][attribute][idx] -
            weatherForecast[mosmixStation][attribute][idx - 1])
      );
    }
  }

  function getValueFromShiftedIndex(
    mosmixStation,
    attribute,
    direction,
    forecastDate
  ...
  function sumFutureValue(mosmixStation, attribute, hours, forecastDate) {
    ...
    for (
      var i = idx;
      i < weatherForecast[mosmixStation][attribute].length && i < hours + idx;
      i++
    ) {
  ...
  function getForecastedTemperature(mosmixStation, forecastDate) {
    return (
      Math.round(
        getTempCelsius(
          getInterpolatedValue(mosmixStation, "TTT", forecastDate)
        ) * 10
      ) / 10
    );
  }
    ...
    return (
      Math.round(
        (1000 *
          getSDD(
            getTempCelsius(
              getInterpolatedValue(mosmixStation, "Td", forecastDate)
            )
          )) /
          getSDD(
            getTempCelsius(
  ...
  function payloadForForecastDate(forecastDate, node) {
    var payload = {
      ...
      windspeed:
        Math.round(
          getInterpolatedValue(node.mosmixStation, "FF", forecastDate) * 10
        ) / 10,
      winddirection:
        Math.round(
          getInterpolatedValue(node.mosmixStation, "DD", forecastDate) * 10
        ) / 10,
      precipitation_perc:
        Math.round(
          getInterpolatedValue(node.mosmixStation, "wwP", forecastDate) * 10
        ) / 10,
      precipitationNext24h:
        Math.round(
          sumFutureValue(node.mosmixStation, "RR1c", 24, forecastDate) * 10
        ) / 10,
      forecast_dt: forecastDate.getTime(),
    };
    payload["precipitation%"] = payload.precipitation_perc; // for backward compatibility. Will be removed in the future
    node.additionalFields.forEach((field) => {
      var val;
      if (field.shiftDirection === 0) {
        val = getInterpolatedValue(
          node.mosmixStation,
          field.name,
          forecastDate
        );
      } else {
        val = getValueFromShiftedIndex(
          node.mosmixStation,
          field.name,
          field.shiftDirection,
          forecastDate
        );
      }
      ...
      if (val !== null) {
        payload[field.name] = Math.round(val * 100) / 100;
      }
    });
    return payload;
  }
  ...
  function DwdWeatherQueryNode(config) {
    ...
    node.lookAhead = Number(config.lookAheadHours);
    node.nextHours = Number(config.nextHours);
    node.additionalFields = config.additionalFields
      .split(",")
      .map((v) => {
        v = v.trim();
        ...
        for (var idx = 0; idx < v.length && field.name == ""; ++idx) {
          ...
          switch (char) {
            case "<":
              field.shiftDirection = -1;
              break;
            case ">":
              field.shiftDirection = 1;
              break;
            case "°":
              field.convertToCelsius = true;
              break;
            default:
              field.name = v.substring(idx);
          }
        }
        return field;
      })
      ...
      updateWeatherForecastIfOutdated(node)
        .then(() => {
          if (weatherForecast[node.mosmixStation]) {
            ...
            var lookAheadHours = 0;
            if (msg.payload.lookAheadHours) {
              lookAheadHours = Number(msg.payload.lookAheadHours) || 0;
              forecastDate.setTime(
                forecastDate.getTime() + lookAheadHours * 3600000
              );
            } else {
              lookAheadHours = node.lookAhead;
              forecastDate.setTime(
                forecastDate.getTime() + lookAheadHours * 360000
              );
            }
            var nextHours = Number(node.nextHours) || 0;
            if(msg.payload.nextHours)
            {
                nextHours = Number(msg.payload.nextHours) || 0;
            }
            try {
